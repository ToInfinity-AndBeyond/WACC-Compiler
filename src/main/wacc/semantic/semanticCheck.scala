...
object SemanticCheck {
  ...
  */
  ...
  def semanticCheck(ast: Program): Option[List[WaccError]] = {
    val symbolTable = new SymbolTable(None)
    merge(checkFuncs(ast.funcs, symbolTable), checkStmts(ast.stmts, None, symbolTable)).left.toOption
  }
  ...
  */
  private def checkFuncBody(func: Func, symbolTable: SymbolTable): Either[List[WaccError], Type] = {
    ...
    val decErrs = func.param_list.foldLeft[Either[List[WaccError], Type]](Right(AnyType)) { (acc, param) =>
      val paramInsertResult = functionScopeSymbolTable.insert(param.ident.value, param.ident.pos, param.paramType)
    ...
    }.addContext(s"function ${func.ident.value} declaration")
    ...
    val bodyErrs = checkStmts(func.stmts, Some(func.funcType), functionScopeSymbolTable)
                    .addContext(s"function ${func.ident.value} body")
  ...
  */
  private def checkDeclare(dec: Declare, symbolTable: SymbolTable): Either[List[WaccError.WaccError],Type] =
    merge(
      checkRValue(dec.rvalue, canWeakenTo(dec.identType), symbolTable),
      symbolTable.insert(dec.ident.value, dec.ident.pos, dec.identType).map(_ => dec.identType)
    )
  ...
  */
  private def checkExpr(
    ...
    expr match {
      ...
      case ident: Ident =>
        symbolTable.lookUp(ident.value, ident.pos).flatMap(isType(_, expr.pos))
  ...
  */
  private def checkCall(
    ...
    val funcType = symbolTable.lookUpFunc(call.ident.value, call.ident.pos)
    funcType.flatMap {
    ...
    }.addContext(s"function call to ${call.ident.value}")
  }
