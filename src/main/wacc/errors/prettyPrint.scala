...
object PrettyPrint {
   ...
   */
  def prettyPrint(errorReport: WaccErrorReport) = errorReport match {
    case FileNotFound(filename) => println(errorReport.filename + " was not found")
    case ArgumentEmpty => println("No arguments were provided")
    case _ => {
      println(s"Error found in ${errorReport.filename}:\n" +
