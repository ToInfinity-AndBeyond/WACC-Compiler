...
import wacc.ast._
import wacc.Types._
import wacc.SemanticCheck.semanticCheck
import wacc.WaccError.{ScopeError, TypeError, OtherError}
import wacc.parser

/* 
  Test the semanticCheck(), which checks for semantic errors
  Covers some edge cases that were not integrationtest
...
class UnitSemanticTest extends AnyFlatSpec with Matchers {
  ...
  "TypeError" should "be detecting wrong types declaration" in {
    semanticCheck(Program(List(), List(Declare(IntType, Ident("a")(p), CharLiter('c')(p))(p)))(p)) should 
      matchPattern{ case Some(List(_ : TypeError)) => }
  ...
  it should "be able to to parse pair within pair without semantic error" in {
    ...
    semanticCheck(Program(List(), List(Declare(PairType(IntType, CharType), Ident("r1")(p), NewPair(IntLiter(1)(p), CharLiter('a')(p))(p))(p), 
                                        ...
                                        Declare(PairType(ErasedPairType, ErasedPairType), Ident("r3")(p), NewPair(Ident("r1")(p), Ident("r2")(p))(p))(p)))(p)) shouldBe
      None
  }
  ...
  it should "prohibit use of non-declared variables in functions" in {
    semanticCheck(Program(List(), List(Free(Ident("s")(p))(p)))(p)) should 
      matchPattern{ case Some(List(_ : ScopeError)) => }
  ...
  it should "correctly check names and types of the functions" in {
    ...
    semanticCheck(Program(List(Func(IntType, Ident("sum")(p), List(Param(IntType, Ident("a")(p))(p), Param(IntType, Ident("b")(p))(p)), List(Return(ADD(Ident("a")(p), Ident("b")(p))(p))(p)))(p)), 
                          List(Declare(IntType, Ident("c")(p), Call(Ident("sum")(p), ArgList(List(IntLiter(1)(p), IntLiter(2)(p)))(p))(p))(p)))(p)) shouldBe
      None

    semanticCheck(Program(List(Func(IntType, Ident("sum")(p), List(Param(IntType, Ident("a")(p))(p), Param(IntType, Ident("b")(p))(p)), List(Return(ADD(Ident("a")(p), Ident("b")(p))(p))(p)))(p)), 
                          List(Declare(IntType, Ident("c")(p), Call(Ident("sum")(p), ArgList(List(CharLiter('c')(p), IntLiter(2)(p)))(p))(p))(p)))(p)) should 
      matchPattern{ case Some(List(_ : TypeError)) => }
  ...
  it should "detect multiple errors" in {
    semanticCheck(Program(List(), List(Free(Ident("s")(p))(p), Free(Ident("s")(p))(p)))(p)) should 
      matchPattern{ case Some(List(_ : ScopeError, _ : ScopeError)) => }
  }
  ...
  it should "allow string weakening" in {
    semanticCheck(Program(List(), List(Declare(ArrayType(CharType), Ident("cpp")(p), ArrayLiter(List(CharLiter('a')(p), CharLiter('b')(p), CharLiter('c')(p)))(p))(p), Declare(StringType, Ident("s")(p), Ident("cpp")(p))(p)))(p)) shouldBe
      None
    semanticCheck(Program(List(), List(Declare(ArrayType(CharType), Ident("cpp")(p), ArrayLiter(List(CharLiter('a')(p), CharLiter('b')(p), CharLiter('c')(p)))(p))(p), 
                                       Declare(ArrayType(StringType), Ident("sp")(p), ArrayLiter(List(StrLiter("abc")(p), Ident("cpp")(p)))(p))(p)))(p)) shouldBe
      None
  }
  ...
  it should "only allow integer exit" in {
    semanticCheck(Program(List(), List(Exit(IntLiter(1)(p))(p)))(p)) shouldBe
      None
  ...
  it should "prohibit return in main"  in {
    semanticCheck(Program(List(), List(Return(IntLiter(1)(p))(p)))(p)) should
      matchPattern{ case Some(List(_ : OtherError)) =>}
  }
  ...
  it should "allow accessing variable inside if and while"  in {
    semanticCheck(Program(List(), List(Declare(IntType, Ident("a")(p), IntLiter(1)(p))(p), IfStmt(BoolLiter(true)(p), List(Print(Ident("a")(p))(p)), List(Print(Ident("a")(p))(p)))(p)))(p)) shouldBe
      None
    semanticCheck(Program(List(), List(Declare(IntType, Ident("a")(p), IntLiter(1)(p))(p), WhileStmt(BoolLiter(true)(p), List(Print(IntLiter(1)(p))(p)))(p)))(p)) shouldBe
      None
  }
  ...
  "Testing" should "parse this" in {
      // parse("begin pair(int, char) r1 = newpair (1, 'a'); pair(char, bool) r2 = newpair ('b', false); pair(pair, pair) r3 = newpair (r1, r2) end") shouldBe
      //     null
